...
public class LargeSquarelotron extends Squarelotron  implements SquarelotronMethods {
	...
	private int[][] to2D(int[] array) {
		int[][] array2D = new int[slength][slength];
		int n = 0; 
		for(int i=0; i < 5; i++) {
	...
	public static Squarelotron makeSquarelotron (int[] array) throws IllegalArgumentException{
		if (array.length !=25){
			throw new IllegalArgumentException("It is not 5x5");
		}
		...
		LargeSquarelotron sSquarelotron = new LargeSquarelotron(array);
		return sSquarelotron;
	...
	public Squarelotron upsideDownFlip(String ring){
		if (ring.equals("outer")){
			...
			for (int i = 0; i < slength; i++){
				for (int j = 0; j<slength; j++){
					...
					else copylotron[k] = squarelotron[4-i][j];
					k++;
				}
			}
		}
			...
			for (int i = 0; i < slength; i++){
				for (int j = 0; j<slength; j++){
					if (i<4 && i>0 && j<4 && j>0){
						copylotron[k] = squarelotron[4-i][j];
					}
					else {
						copylotron[k] = squarelotron[i][j];			
					}
		...
		LargeSquarelotron flipSquarelotron = new LargeSquarelotron(copylotron);
		...
		if (ring.equals("outer")){
			...
			for (int i = 0; i < slength; i++){
				for (int j = 0; j < slength; j++){
					...
					else copylotron[k] = squarelotron[i][4-j];
					k++;
				}
			}
		}
			...
			for (int i = 0; i < slength; i++){
				for (int j = 0; j<slength; j++){
					if  ((i<4 && i>0) && (j<4 && j>0)){
						copylotron[k] = squarelotron[i][4-j];
					}
		...
		if (ring.equals("outer")){
			...
			for (int i = 0; i < slength; i++){
				for (int j = 0; j<slength; j++){
					...
					else copylotron[k] = squarelotron[4-j][4-i];
					k++;
				}
			}
		}
			...
			for (int i = 0; i < slength; i++){
				for (int j = 0; j<slength; j++){
					if (i<4 && i>0 && j<4 && j>0){
						copylotron[k] = squarelotron[4-j][4-i];
					}
					else copylotron[k] = squarelotron[i][j];
					k++;
				}
	...
	public Squarelotron sideFlip(String ring){
		int k = 0;
			for (int i = 0; i < slength; i++){
				for (int j = 0; j<slength; j++){
					copylotron[k] = squarelotron[i][j];
					if ((i==0 && ring.equals("top")) || (i==3 && ring.equals("bottom"))){
						copylotron[k] = squarelotron[i+1][j];
					}
					if ((i==1 && ring.equals("top")) || (i==4 && ring.equals("bottom"))){
						copylotron[k] = squarelotron[i-1][j];
					}
					if ((j==3 && ring.equals("right"))|| (j==0 && ring.equals("left"))){
						copylotron[k] = squarelotron[i][j+1];
					}
					if ((j==4 && ring.equals("right"))|| (j==1 && ring.equals("left"))){
							copylotron[k] = squarelotron[i][j-1];
					}
					k++;
					}
			}
			return makeSquarelotron (copylotron);
		}
	...
	/**
	 * Makes a good representation of the squarelotron (matrix)
	 */
	@Override
